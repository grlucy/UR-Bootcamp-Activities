...
module.exports = function(app) {
  ...
  app.get("/api/:book", function(req, res) {
    Book.findAll({ where: { title: req.params.book } }).then(result => {
      res.json(result);
    });
  });
  ...
  app.get("/api/genre/:genre", function(req, res) {
    Book.findAll({ where: { genre: req.params.genre } }).then(result => {
      res.json(result);
    });
  });
  ...
  app.get("/api/author/:author", function(req, res) {
    Book.findAll({ where: { author: req.params.author } }).then(result => {
      res.json(result);
    });
  });
  ...
  app.get("/api/books/long", function(req, res) {
    Book.findAll({ where: { pages: { $gt: 150 } } }).then(result => {
      res.json(result);
    });
  });
  ...
  app.get("/api/books/short", function(req, res) {
    Book.findAll({ where: { pages: { $lte: 150 } } }).then(result => {
      res.json(result);
    });
  });
