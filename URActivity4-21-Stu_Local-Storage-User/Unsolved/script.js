...
function renderLastRegistered() {
  ...
  let storedEmail = localStorage.getItem("Stored Email");
  let storedPassword = localStorage.getItem("Stored Password");

  if (storedEmail === null || storedPassword === null) {
    return;
  } else {
    document.getElementById("user-email").textContent = storedEmail;
    document.getElementById("user-password").textContent = storedPassword;
  }
}
...
signUpButton.addEventListener("click", function(event) {
  ...
  if (email === "") {
    ...
    localStorage.setItem("Stored Password", password);
